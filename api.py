"""
E-Trade Foreign Share Working — FastAPI Backend
...
app = FastAPI(title="E-Trade Foreign Share Working API", version="1.0.0")
...
# ---------------------------------------------------------------------------
# Helpers (ported 1-to-1 from ETradeApp)
# ---------------------------------------------------------------------------

def find_col(headers: dict, keywords: list) -> Optional[int]:
...
def copy_row_style(ws, src: int, dst: int):
...
def extract_dividends(pdf_files: list, start_dt: datetime) -> pd.DataFrame:
    """
    pdf_files: list of file-like objects (already read into BytesIO).
    """
    recs = []
...
def build_report(
    ...
    gl_files: list,          # list of BytesIO
    pdf_files: list,         # list of BytesIO
    client_name: str,
    pan_number: str,
    fy: str,
) -> bytes:
    ...
    try:
        ...
        # ---- CAPITAL GAINS ----
        if gl_files and "Capital Gains" in wb.sheetnames:
            ws = wb["Capital Gains"]

            frames = []
            for gl_bytes in gl_files:
                frames.append(pd.read_excel(gl_bytes))
            ...
            df_f = df_cg[(df_cg["Date Sold"] >= start_dt) & (df_cg["Date Sold"] <= end_dt)]
            ...
            headers = {
                str(ws.cell(h_row, c).value).strip(): c
                for c in range(1, ws.max_column + 1)
                if ws.cell(h_row, c).value
            }
        ...
        # ---- DIVIDENDS ----
        if pdf_files and "Dividends" in wb.sheetnames:
            ws = wb["Dividends"]

            df_div = extract_dividends(pdf_files, start_dt)
...
@app.post("/generate")
async def generate(
    template:    UploadFile         = File(...,  description="Master template .xlsx"),
    gl_files:    List[UploadFile]   = File([],   description="G&L Excel files"),
    pdf_files:   List[UploadFile]   = File([],   description="Statement PDF files"),
    ...
    gl_io  = [io.BytesIO(await f.read()) for f in gl_files]
    pdf_io = [io.BytesIO(await f.read()) for f in pdf_files]
...
async def preview_gl(
    ...
    """Returns a JSON preview of filtered capital-gains rows."""
    if fy not in FY_MAP:
    ...
    return {"rows": df.fillna("").to_dict("records"), "count": len(df)}
...
async def preview_dividends(
    ...
    """Returns a JSON preview of extracted dividend rows."""
    if fy not in FY_MAP:
        raise HTTPException(status_code=400, detail="Unknown FY")

    start_dt, end_dt = map(pd.to_datetime, FY_MAP[fy])
    pdf_io = [io.BytesIO(await f.read()) for f in pdf_files]
    df = extract_dividends(pdf_io, start_dt)
