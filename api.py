"""
E-Trade Foreign Share Working — FastAPI Backend (Low Memory)
...
app = FastAPI(title="E-Trade Foreign Share Working API", version="3.0.0")
...
def extract_dividends(pdf_bytes_list, start_dt):
    """Extract dividends using pypdf (low memory)."""
    recs = []
...
def build_report(template_bytes, gl_bytes_list, pdf_bytes_list, client_name, pan_number, fy):
    ...
    try:
        ...
        if gl_bytes_list and "Capital Gains" in wb.sheetnames:
            ws = wb["Capital Gains"]
            frames = [pd.read_excel(io.BytesIO(b)) for b in gl_bytes_list]
            del gl_bytes_list
            gc.collect()
            ...
            for i, row in enumerate(rows):
                ...
                if c_sd:  ws.cell(curr, c_sd).value = row.get("Date Sold");  ws.cell(curr, c_sd).number_format = "DD-MM-YYYY"
                if c_pd:  ws.cell(curr, c_pd).value = row.get("Date Acquired"); ws.cell(curr, c_pd).number_format = "DD-MM-YYYY"
        ...
        if pdf_bytes_list and "Dividends" in wb.sheetnames:
            ws = wb["Dividends"]
            df_div = extract_dividends(pdf_bytes_list, start_dt)
            del pdf_bytes_list; gc.collect()
            ...
            for i, row in enumerate(rows):
                curr = data_start + i
                copy_row_style(ws, data_start, curr)
                if d_dt:  ws.cell(curr, d_dt).value = row.get("Date"); ws.cell(curr, d_dt).number_format = "DD-MM-YYYY"
...
async def generate(
    ...
    try:
        xlsx_bytes = build_report(template_bytes, gl_bytes_list, pdf_bytes_list, client_name, pan_number, fy)
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f'attachment; filename="Foreign_Shares_{safe_name}.xlsx"'},
    )

@app.post("/preview-gl")
async def preview_gl(gl_files: List[UploadFile] = File(...), fy: str = Form(...)):
    if fy not in FY_MAP:
        raise HTTPException(status_code=400, detail="Unknown FY")
    start_dt, end_dt = map(pd.to_datetime, FY_MAP[fy])
    frames = [pd.read_excel(io.BytesIO(await f.read())) for f in gl_files]
...
async def preview_dividends(pdf_files: List[UploadFile] = File(...), fy: str = Form(...)):
    if fy not in FY_MAP:
        raise HTTPException(status_code=400, detail="Unknown FY")
    start_dt, end_dt = map(pd.to_datetime, FY_MAP[fy])
    pdf_bytes_list = [await f.read() for f in pdf_files]
    df = extract_dividends(pdf_bytes_list, start_dt)
    del pdf_bytes_list; gc.collect()
